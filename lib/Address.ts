...
export class Address {
  ...
  public toLegacyAddress(address: string): string {
    ...
    let tent: CoinInfo = Bitcoin.networks.tent
    switch (prefix) {
      case "tent":
    ...
    let version: number = tent.pubKeyHash
    switch (type) {
      case "P2PKH":
  ...
  public fromXPub(xpub: string, path: string = "0/0"): string {
    ...
    if (xpub[0] === "x") bitcoincash = Bitcoin.networks.tent
  ...
  public fromXPriv(xpriv: string, path: string = "0'/0"): string {
    let bitcoincash: CoinInfo
    if (xpriv[0] === "x") bitcoincash = Bitcoin.networks.tent
    else bitcoincash = Bitcoin.networks.tentTest
